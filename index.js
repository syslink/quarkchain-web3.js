...
function loadContract(abi, contractAddress, web3in, web3http) {
  ...
  web3contract.eth.call = (obj, callback) => {
    ...
    Object.defineProperty(web3in, 'qkc', {
      value: {
        ...
        async sendTransaction(obj) {
          ...
          rawTx.nonce = await web3http.eth.getTransactionCount(fromEthAddress, rawTx.fromFullShardKey);
          if (!rawTx.networkId) {
            // default network is devnet
            rawTx.networkId = '0xee5';
          }
          ...
          return web3http.eth.sendRawTransaction(payload);
        },
        ...
        contract(abi) {
          const contractFactory = new web3in.eth.Contract(abi);
          //const originalFactory = new web3in.eth.Contract(abi);
